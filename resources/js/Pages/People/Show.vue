<script setup>
    ...
    const getTasks = () => {
        axios.get(route('api.task.show', ['person', person.id])).then((response) => {
            tasks.value =  response.data;
        });
    };
    const submit = () => {
        form.post(route('person.tasks.store', person.id), {
            preserveScroll: true,
            onSuccess: () => {
                getTasks();
            },
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Person: {{ person.pro_fullname }}</h2>
        </template>
                        ...
                        <div class="bg-white overflow-hidden shadow rounded-lg border">
                            <div class="px-4 py-5 sm:px-6">
                                ...
                                <p class="mt-1 max-w-2xl text-sm text-gray-500">
                                    This is some information about the person.
                                </p>
                            </div>
                            <div class="border-t border-gray-200 px-4 py-5 sm:p-0">
                                <dl class="sm:divide-y sm:divide-gray-200">
                                    ...
                                    <div class="py-3 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                        ...
                                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                            <span class="bg-blue-100 text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full dark:bg-blue-900 dark:text-blue-300">{{ person.business_name }}</span>
                                        </dd>
                                    </div>
                                    <div class="py-3 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                        <dt class="text-sm font-medium text-gray-500">
                    ...
                    <div class="border-t border-gray-200 px-4 py-5 m-5 sm:p-0 grid grid-cols-6 gap-6">
                        <form @submit.prevent="submit" class="p-3.5 col-span-3 sm:col-span-2">
                            <div class="max-w-xl mx-auto mt-16 flex w-full flex-col border rounded-lg bg-white p-8">
                                ...
                                <p class="mb-5 leading-relaxed text-gray-600 lg:text-sm">Want to add some task for the person?</p>
                                <div class="mb-4">
